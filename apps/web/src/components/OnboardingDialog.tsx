...
export function OnboardingDialog({ onComplete }: { onComplete: () => void }) {
  ...
  return <ModalSurface onClose={onComplete} labelledBy="onboarding-title" testId="onboarding-dialog" panelClassName="w-full max-w-xl">
      ...
      <div className="p-6">
        ...
        <div className="mt-5 flex justify-center gap-1.5" role="progressbar" aria-label={t("onboardingProgress")} aria-valuemin={1} aria-valuemax={steps.length} aria-valuenow={index + 1}>{steps.map((item, itemIndex) => <span key={item.key} className={`h-1.5 rounded-full transition-all ${itemIndex === index ? "w-8 bg-primary" : "w-2 bg-border"}`} />)}</div>
      </div>
