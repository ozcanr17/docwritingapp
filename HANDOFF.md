...
**Migrations (8):** `init`, `hierarchy_prefix_indexes`, `suspect_links`, `platform_capabilities`, `test_step_result`, `requirement_numbers`, `stable_object_numbers`, `profiles_and_numbering`.

**Test status (all green as of 2026-07-15):** api **46**, worker **12**, web **18**. `pnpm verify` passes database validation, TypeScript checking, lint, forbidden-character scan, all **76** unit/integration tests and the production build with bundle budgets. Multi-document coverage includes tab deduplication/activation, adjacent selection on close, split-state invariants, primary/secondary focus swapping and tab-bar actions. The measured initial web graph is **95.0 KiB gzip** and the largest lazy chunk remains 72.1 KiB gzip; the Tauri window API is a separate 4.0 KiB lazy chunk. The existing Playwright e2e suite has **7** flows (`smoke`, `editor`, `exports`, `columns`, `traceability`, `accessibility`, `desktop-auth`) but was not rerun for this delivery. Local interactive browser verification was attempted but the browser surface rejected navigation from its prior connection-error page under URL policy; no alternate browser workaround was used. The reference requirement/test DOCX samples were rendered to page images and audited with no clipping, overlap or accessibility finding. Tauri Rust was not rechecked because `cargo` is not installed in the active shell; GitHub's desktop matrix is the authoritative native window/capability gate. The 10,000-row benchmark previously seeded in 768.2 ms and measured a 290.8 ms outline p95 with a 4.33 MiB response, below its 2,500 ms budget.
...
5. Extend the workspace-tab system with drag reordering, optional user-scoped tab-session restore, adjustable split ratio and three-pane layouts only if real workflows justify the added density.
